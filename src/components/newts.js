...
import { Search, paginatePageReset, paginatePageNext, paginatePagePrev, resetErrors} from '../actions/authentication';
import classnames from 'classnames';
import {Bootstrap,Form, Row, Col, Button, FormGroup, FormControl, Table} from 'react-bootstrap';
  ...
  render() {
    ...
    return (
      ...
      {filter.map((detail,index) =>{
        return (
          <tr value={detail.email} onClick={() => this.handleClick1(detail.email)}>
          <td value={detail.name} onClick={() => this.handleClick2(detail.name)}>{detail.name}</td>
          <td>{detail.email}</td>
          <td>{detail.role}</td>
...
export default connect(mapStateToProps,{ Search, paginatePageReset, paginatePageNext, paginatePagePrev, resetErrors })(newts)